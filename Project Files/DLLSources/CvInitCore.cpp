...
#include "autogenerated/AutoGitVersion.h"
...
{
	FASSERT_BOUNDS(0, MAX_PLAYERS, eID, "CvInitCore::getSlotVacant");
	...
	if ( checkBounds(eID, 0, MAX_PLAYERS) )
	{
		...
		if ( (eStatus == SS_OPEN) || (bTakeoverAI && (eStatus == SS_COMPUTER) && getPlayableCiv(eID)) )
		{
...
{
	FASSERT_BOUNDS(0, MAX_PLAYERS, eOldID, "CvInitCore::reassignPlayer (eOldID)");
	FASSERT_BOUNDS(0, MAX_PLAYERS, eNewID, "CvInitCore::reassignPlayer (eNewID)");

	if ( checkBounds(eOldID, 0, MAX_PLAYERS) && checkBounds(eNewID, 0, MAX_PLAYERS) )
	{
...
void CvInitCore::refreshVictories()
{
	...
	if (m_iNumVictories > 0)
	{
		...
		{
			m_abVictories[i] = GC.getVictoryInfo((VictoryTypes) i).getDefault();
		}
	}
}
...
void CvInitCore::setLeaderName(PlayerTypes eID, const CvWString & szLeaderName)
{
	...
	if ( checkBounds(eID, 0, MAX_PLAYERS) )
	{
		...
		gDLL->getInterfaceIFace()->dirtyTurnLog(eID);
...
int CvInitCore::getMaxEuropePlayers() const
{
	int iNumEuropes = 0;

	for (int iCiv = 0; iCiv < GC.getNumCivilizationInfos(); ++iCiv)
	{
		if (GC.getCivilizationInfo((CivilizationTypes) iCiv).isEurope())
		{
			++iNumEuropes;
		}
	}
...
{
	CvString szRevision;
	pStream->ReadString(szRevision);

	unsigned int uiSavegameVersion = 0;
	pStream->Read(&uiSavegameVersion);
	
	SavegameAlert::HandleSavegameTooOld(uiSavegameVersion < SAVEGAME_OLDEST_COMPATIBLE_VERSION);
	SavegameAlert::HandleSavegameTooNew(uiSavegameVersion > SAVEGAME_VERSION);
	...
	if (CvPlayerAI::areStaticsInitialized())
	{
		for (PlayerTypes ePlayer = FIRST_PLAYER; ePlayer < NUM_PLAYER_TYPES; ++ePlayer)
		{
			...
			kPlayer.updateHuman();
			kPlayer.updateTeamType();
		}
	}

	if (reader.getSavegameVersion() < 2)
	{
...
{
	CvString szRevision = szGitVersion;
	pStream->WriteString(szRevision);

	pStream->Write(SAVEGAME_VERSION);
